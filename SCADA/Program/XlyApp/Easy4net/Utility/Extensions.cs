...
public static class MyExtensions
{
    ...
    {
        Regex regex = new Regex("^1[3|5|7|8]\\d{9}$");
        return regex.IsMatch(input);
    }
    /// <summary>
    /// 判断一个整型是否包含在指定的值内
    ...
    //public static float ToFloat(this string str, int digits)
    //{
    //    float test;
    //    return float.TryParse(str, out test) ? test.Round(digits) : 0;
    //}
    public static float ToFloat(this string str)
    {
        float test;
        return float.TryParse(str, out test) ? test : 0;
    }
    ...
    public static string DateDiff(DateTime DateTime1, DateTime DateTime2)
    {
        ...
        try
        {
            ...
            if (ts.Days >= 1)
            {
                dateDiff = ts.Days.ToString() + "天" + ts.Hours.ToString() + "小时" + ts.Minutes.ToString() + "分";
            }
            else
            {
                if (ts.Hours >= 1)
                {
                    dateDiff = ts.Hours.ToString() + "小时" + ts.Minutes.ToString() + "分";
                }
                else
                {
                    if (ts.Minutes >= 1)
