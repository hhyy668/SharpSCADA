...
namespace Easy4net.Context
{
    ...
    public class Session
    {
        ...
        public static Session NewInstance(string connName)
        {
            ...
            {
                session.ConnectDB("SqlServerString");//如果未指定连接数据库名称，则直接连接sqlserver
            }
        ...
        public int Insert<T>(T entity)
        {
            ...
            try
            {
                ...
                //执行Insert命令
                if (tableInfo.Strategy == GenerationType.FILL)
                {
        ...
        /// <returns></returns>
        public string ExcuteStoredProcReturnString(string storedProcName, SqlParameter[] paras)
        {
            IDataReader idr;
            IDbTransaction transaction = null;
            IDbConnection connection = null;
            try
            {
                ...
                idr = AdoHelper.ExecuteReader(dbFactory, CommandType.StoredProcedure, "Com_GetMaintainSeq", paras);
                {
                    while (idr.Read())
                    {
                        return idr.GetString(0);
                    }
                }
            }
            catch (Exception e)
        ...
        /// <param name="paras"></param>
        /// <returns></returns>
        public DataSet ExcuteStoredProc(string storedProcName, SqlParameter[] paras)
        {
            ...
            try
            {
                ...
                connection = GetConnection();
                transaction = GetTransaction();
                dataSet = AdoHelper.dataSet(dbFactory, CommandType.StoredProcedure, storedProcName, paras);
                return dataSet;
            }
            catch (Exception e)
            {
        ...
        /// <param name="paras"></param>
        /// <returns></returns>
        public DataSet ExcuteSQLReturnDataSet(string strSQL)
        {
            DataSet dataSet = new DataSet();
            IDbTransaction transaction = null;
            IDbConnection connection = null;
            try
            {
                ...
                connection = GetConnection();
                transaction = GetTransaction();
                dataSet = AdoHelper.dataSet(dbFactory, CommandType.Text, strSQL);
                return dataSet;
            }
            catch (Exception e)
            {
